...
def points_projection(points: Float[Tensor, "B Np 3"],
    ...
    # print(fragments_idx.shape)
    # print(fragments_idx.max())
    ...
    # print(local_features.shape) #torch.Size([1, 773, 256, 256]) 
    local_features = local_features.permute(0, 2, 3, 1).unsqueeze(-2).expand(-1, -1, -1, R, -1)  # (B, H, W, R, C)
    # print("local_features")
    # print(local_features.shape) #([1, 256, 256, 1, 773])
    ...
    print("local_features_proj")
    print(local_features_proj.shape) 

    local_features_proj = local_features_proj.reshape(B, Np, C)
    print(local_features_proj.shape) #e([1, 6186, 773])
    return local_features_proj

def points_projection_my(points: Float[Tensor, "B Np 3"],
                      c2ws: Float[Tensor, "B 4 4"],
    ...
    cameras =  PerspectiveCameras(device=device, R=input_cam['input_R'].float(),T=input_cam['input_T'].float(),
    ...
    local_features_proj[points_to_visible_pixels] = local_features[visible_pixels]
    local_features_proj = local_features_proj.reshape(B, Np, C)
    # local_features_proj = local_features_proj.reshape(B, Np, 3)
    # print(local_features_proj.shape)
    # print(local_features_proj.max())
    # print("ok!!!!!!!")
    # mesh_v = trimesh.Trimesh(vertices=points[0].detach().cpu().numpy(), faces=face[0].detach().cpu().numpy(), process=False)
    # mesh_v.visual.vertex_colors = local_features_proj[0].detach().cpu().numpy()
    # # trimesh.visual.color.ColorVisuals(mesh=mesh_v, vertex_colors=local_features_proj[0].detach().cpu().numpy()*255)
    # mesh_v.export('mesh_vis_2w.obj')
    return local_features_proj

def points_projection_fs(points: Float[Tensor, "B Np 3"],
                      c2ws: Float[Tensor, "B 4 4"],
                      w2cs: Float[Tensor, "B 4 4"],
                      intrinsics: Float[Tensor, "B 3 3"],
                      input_cam,
                      ...
                      face,
                      raster_point_radius: float = 0.0075,  # point size
                      raster_points_per_pixel: int = 3,  # a single point per pixel, for now
                      bin_size: int = 0):
    """
    points: (B, Np, 3)
    """
