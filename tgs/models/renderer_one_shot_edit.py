...
class GS3DRenderer(BaseModule):
    ...
    def forward_single_batch(
        ...
        color_w0 = torch.ones(size=(16,3,1024,2048))
        color_w0[0,...,:1024]=color_w.view(-1,16,3)[0,0,:].unsqueeze(-1).unsqueeze(-1).repeat(1,1024,1024)
        color_w0[1,...,:1024]=color_w.view(-1,16,3)[0,1,:].unsqueeze(-1).unsqueeze(-1).repeat(1,1024,1024)
        color_w0[0,...,1024:]=color_w.view(-1,16,3)[0,2,:].unsqueeze(-1).unsqueeze(-1).repeat(1,1024,1024)
        color_w0[1,...,1024:]=color_w.view(-1,16,3)[0,3,:].unsqueeze(-1).unsqueeze(-1).repeat(1,1024,1024)
