...
class LocalPoolPointnet(BaseModule):
    ...
    def forward(self, p):
        ...
        position = p[..., :3]
        position=position-position.mean(-2).unsqueeze(-2)
        position = torch.clamp(position, -self.cfg.radius + 1e-6, self.cfg.radius - 1e-6)
        position_norm = scale_tensor(position, (-self.cfg.radius, self.cfg.radius), (0, 1))
        # print("position_norm")
        # print(position_norm[...,0].max()) #-0.6366 0.4560
        # print(position_norm[...,0].min()) #-0.8444 -0.7492
        # print(position_norm[...,1].max()) #-0.6414 0.6287
        # print(position_norm[...,1].min()) #-0.8679 -0.6993
        # print(position_norm[...,2].max()) #0.8546 0.9557
        # print(position_norm[...,2].min()) #0.5718 -0.6879
